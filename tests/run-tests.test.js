...
test('runTests: GH_MAESTRO_WORKSPACE があっても cwd を executionWorkspace としてテストを実行し成果物を書く', () => {
  const worktree = tempWorktree();
  const parentWs = tempWorktree();
  const fixture = runWithChild({
    cwd: worktree,
    extraDeps: {},
    actor: 'poll-pr',
  ...
  // GH_MAESTRO_WORKSPACE を環境変数に渡して直接呼ぶ
  const writes = [];
  const calls = [];
  const result = runTests(
    {
      ...
      env: {
        TEST_RUNNER_FIXTURE: '1',
        GH_MAESTRO_WORKSPACE: parentWs,
      },
    },
    {
      clearArtifactFn: (wt) => calls.push({ type: 'clear', worktree: wt }),
      resolveGitHeadFn: (wt) => {
        calls.push({ type: 'head', worktree: wt });
        return SHA;
      },
      calculateWorktreeContentHashFn: (wt) => {
        calls.push({ type: 'hash', worktree: wt });
        return CONTENT_HASH;
      },
      spawnSyncFn: (command, args, options) => {
        calls.push({ type: 'spawn', command, args, options });
      ...
      writeArtifactFn: (wt, artifact) => {
        writes.push({ worktree: wt, artifact });
      },
    },
  );
  ...
  assert.equal(writes[0].worktree, worktree);
  assert.deepEqual(
    calls.filter((c) => ['clear', 'head', 'hash'].includes(c.type)).map((c) => c.worktree),
    [worktree, worktree, worktree],
  );
  assert.equal(calls.find((c) => c.type === 'spawn').options.cwd, worktree);
});
