...
test('normalizeWorkerEntry: 新形式 { paneId, agentId } をそのまま読む', () => {
  ...
  assert.deepEqual(r, { paneId: '42', agentId: 'agy', notifierPid: null });
});

test('normalizeWorkerEntry: 旧形式（pane_id文字列のみ）を後方互換で読む', () => {
  const r = normalizeWorkerEntry('42');
  assert.deepEqual(r, { paneId: '42', agentId: null, notifierPid: null });
});
...
test('normalizeWorkerEntry: undefined は paneId: null を返す', () => {
  ...
  assert.deepEqual(r, { paneId: null, agentId: null, notifierPid: null });
});
...
test('normalizeWorkerEntry: notifierPid を保持する', () => {
  ...
  assert.deepEqual(r, { paneId: '42', agentId: 'agy', notifierPid: 1234 });
});
