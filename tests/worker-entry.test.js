...
test('normalizeWorkerEntry: 新形式 { paneId, agentId } をそのまま読む', () => {
  ...
  assert.deepEqual(r, { paneId: '42', agentId: 'agy', issue: null, skill: null, notifierPid: null });
});

test('normalizeWorkerEntry: 旧形式（pane_id文字列のみ）を後方互換で読む', () => {
  const r = normalizeWorkerEntry('42');
  assert.deepEqual(r, { paneId: '42', agentId: null, issue: null, skill: null, notifierPid: null });
});
...
test('normalizeWorkerEntry: undefined は paneId: null を返す', () => {
  ...
  assert.deepEqual(r, { paneId: null, agentId: null, issue: null, skill: null, notifierPid: null });
});
...
test('normalizeWorkerEntry: notifierPid を保持する', () => {
  ...
  assert.deepEqual(r, { paneId: '42', agentId: 'agy', issue: null, skill: null, notifierPid: 1234 });
});
...
test('normalizeWorkerEntry: issue を数値化して保持する（文字列で渡されても Number() される）', () => {
  ...
  assert.deepEqual(r, { paneId: '42', agentId: 'agy', issue: 51, skill: null, notifierPid: null });
});

test('normalizeWorkerEntry: skill を保持する', () => {
  const r = normalizeWorkerEntry({ paneId: '42', agentId: 'claude-ds', issue: 7, skill: 'gh-maestro-coder' });
  assert.deepEqual(r, { paneId: '42', agentId: 'claude-ds', issue: 7, skill: 'gh-maestro-coder', notifierPid: null });
});
