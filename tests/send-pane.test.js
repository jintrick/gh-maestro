...
test('本文は.gh-maestro/messages/にファイルとして書かれ、paneには短い通知だけが送られる', () => {
  withTempDir(workspace => {
    fs.mkdirSync(path.join(workspace, '.gh-maestro'), { recursive: true });
    fs.writeFileSync(
      path.join(workspace, '.gh-maestro', 'workers.json'),
    ...
    const logPath = path.join(workspace, 'send-text-calls.log');
    const cwdJson = `[{"pane_id":0,"cwd":"${pathToFileURL(workspace).href}"}]`;
    // "@"はClaude Codeのファイルメンション補完を誘発しうる特殊文字の代表例
    const specialMessage = '修正依頼: 担当者user@example.comへ確認してください。/コマンド風の文字列も含む。';

    const r = run(['orchestrator', specialMessage, '--workspace', workspace], {
      WEZTERM_PANE: '1',
      WEZTERM_MOCK: writeLoggingWeztermMock(workspace, logPath),
      MOCK_LIST_JSON: cwdJson,
    });
    assert.equal(r.status, 0, `expected exit 0, got ${r.status}: ${r.stderr}`);
    ...
    const calls = fs.readFileSync(logPath, 'utf8').trim().split('\n').map(l => JSON.parse(l));
    const sendTextCalls = calls.filter(c => c[0] === 'cli' && c[1] === 'send-text');
    assert.equal(sendTextCalls.length, 2, '通知送信 + Enter送信の2回のはず');

    const sentText = sendTextCalls[0][sendTextCalls[0].length - 1];
    assert.ok(!sentText.includes('@'), `paneへの送信テキストに"@"が含まれてはならない: ${sentText}`);
    assert.ok(!sentText.includes(specialMessage), 'paneへは本文そのものではなく通知文が送られるはず');
    assert.match(sentText, /を読んでください。$/);

    const messagesDir = path.join(workspace, '.gh-maestro', 'messages');
    const files = fs.readdirSync(messagesDir).filter(f => f.startsWith('msg-'));
    assert.equal(files.length, 1, 'メッセージファイルが1つ書かれているはず');
    const written = fs.readFileSync(path.join(messagesDir, files[0]), 'utf8');
    assert.ok(written.includes(specialMessage), 'ファイルには本文がそのまま書かれているはず');
    assert.ok(sentText.includes(path.join(messagesDir, files[0])), '通知にファイルの絶対パスが含まれるはず');
  });
});
