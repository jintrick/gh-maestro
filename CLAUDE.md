...
PR が opened/reopened されると単一の AI レビュアー（`reviewer.md`）が起動し、correctness・maintainability・resilience の3観点をまとめてレビューして GitHub にコメントを投稿する仕組み。対象リポジトリへのデプロイは `node scripts/setup-ai-review.js <owner/repo>`。

仕様: `workflows/SPEC.md`。編集・変更前に必ず読むこと。

### `gh aw compile` はユーザーが実行する

`gh aw compile -d workflows` は Claude Code から実行すると必ず失敗する。`workflows/reviewer.md` を編集した後は、ユーザーに以下を依頼すること：
