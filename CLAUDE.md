...
詳細仕様: **`workflows/SPEC.md`**

### ファイル構成

| パス | 役割 |
|---|---|
| `workflows/reviewer-*.md` | gh-aw ソース（人間が編集する） |
| `workflows/reviewer-*.lock.yml` | gh-aw コンパイル出力（編集禁止） |
| `.github/workflows/reviewer-*.yml` | lock.yml のコピー（直接編集しない） |
| `.github/workflows/shared/reviewer-output-policy.md` | 共有フォーマットポリシー（runtime-import で読込） |
| `workflows/caller-template/ai-review.yml` | エントリポイントのテンプレート |

### コンパイル手順

フロントマターやプロンプトボディを変更したとき：

```bash
gh aw compile -d workflows
cp workflows/reviewer-*.lock.yml .github/workflows/
# コピー後にファイル名から .lock を除く（例: reviewer-correctness.lock.yml → reviewer-correctness.yml）
```

フォーマットポリシー（`.github/workflows/shared/reviewer-output-policy.md`）の変更は**コンパイル不要**。

### runtime-import

lock ファイルに `{{#runtime-import}}` マクロが記録される。**`{{#include}}` は廃止済み**（独自構文だった）。
...
CI 実行時に `.github/workflows/shared/reviewer-output-policy.md` として解決される（`setup-ai-review.js` がデプロイする）。

### デプロイ

```bash
node scripts/setup-ai-review.js <owner/repo>
```

`buildManifest()` で定義されたすべてのファイルを対象リポジトリの `.github/workflows/` に GitHub API 経由でデプロイする。`shared/` 以下も含む。

### 禁止事項

- `synchronize` トリガーを `ai-review.yml` に追加しない（再レビューは close→reopen）
- `.github/workflows/reviewer-*.yml` を直接編集しない（lock.yml のコピーが正）
- `{{#include}}` を使わない（廃止済み）
