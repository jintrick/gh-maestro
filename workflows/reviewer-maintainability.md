...
description: PR Maintainability Reviewer - 将来の変更で事故が起きる場所を探す
on:
  workflow_call:
    inputs:
      pr_number:
...
あなたは Maintainability Reviewer である。レビュー対象PR番号: ${{ inputs.pr_number }}
...
`gh pr view ${{ inputs.pr_number }} --json files` で変更ファイル一覧を取得し、命名・構造リスクの高いファイル（`utils`, `common`, `helpers` を含むパス、新規作成ファイル）から順にレビューせよ。
...
```json
{
  "pull_request_number": ${{ inputs.pr_number }},
  "event": "APPROVE",
  "body": "LGTM — maintainability review: 保守性リスクなし"
}
```
