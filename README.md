...
├── skills/
│   ├── gh-maestro/{claude,agy}/SKILL.md          # 起動スキル
│   ├── gh-maestro-orchestrator/{claude,agy}/SKILL.md
│   │   └── scripts/            # 共通アセットスクリプト
...
│   ├── gh-maestro-coder/{claude,agy}/SKILL.md
│   │   └── scripts/send-pane.js
│   ├── gh-maestro-reviewer/{claude,agy}/SKILL.md
│   │   └── scripts/send-pane.js
│   └── gh-maestro-base/{claude,agy}/SKILL.md
│       └── scripts/send-pane.js
└── docs/rag/
    ├── claude-code/            # Claude Code RAGドキュメント
    ├── antigravity/            # agy RAGドキュメント
    └── wezterm/                # WezTerm RAGドキュメント
```
...
1. スキルを `~/.claude/skills/` および `~/.gemini/antigravity-cli/skills/` にエージェント別で配置
2. `gh-maestro-setup.js` を `~/.gh-maestro/scripts/` に配置
...
`skills/<skill>/<agent>/SKILL.md` を編集してインストーラーを再実行すると更新が反映される。新しいワーカー役職を追加する場合は `skills/` に新しいスキルディレクトリを追加し、対応するエージェントの `SKILL.md` を作成してインストールするだけでよい。
