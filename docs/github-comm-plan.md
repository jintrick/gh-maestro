...
- 受信は `gh api repos/{repo}/issues/{N}/comments --paginate` で**全件取得**する。`since` フィルタは使わない（時刻カーソルによる既読推測は廃止。取得範囲外に未記録IDを作らないことが保証できないため）。
...
- `sinceByIssue`（診断情報のみ）: 既読判定・取得フィルタには**一切使わない**。
