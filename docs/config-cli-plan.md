...
3. **安全性の担保**: 手動書き換えでは、エージェントIDのタイポや、ワークスペース固有の設定におけるコマンド上書き禁止制約（`scripts/shared/resolve-config.js`）に起因するミスに気づきにくい課題があります。
...
- **エージェント疎通検証**: 
  現在割り当てられている各エージェントの起動コマンド（`command`）が、システム上で実行可能（`scripts/shared/agent-exec.js`の`checkAgentExists` でパスが通っているか）かをチェックし、`[✓]` または `[✗]` で可視化します。
- **セキュリティ・制約の可視化**: 
...
3. **`status` コマンドの実装**
   - `scripts/agent-defaults.json` と `config.json` のマージ状況の算出。
