...
- 新しくプレースホルダや置換を足す際は、`skills/agents.yaml`の各エージェントのsubstitutionsで、対応するツール（Monitor等）を持たないエージェント向けの代替文言（session-resume系: worker-supervisor.js経由）が定義されているか確認する。
