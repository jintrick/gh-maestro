...
- `reviewArtifactPath(ghDir, pr, '.log')` だけは例外で `ghDir` 直下ではなく `shared/headless-launch.js` の `workerLogPath()` と同じ `<workspace>/.gh-maestro/worker-logs/` に置く（通常ワーカーとログ保存先を統一するため）。`.running`/`.json` は対象外で `ghDir` 直下のまま。
...
- `manager.running` の所有者は `run-review-manager.js` の `process.pid` ただ一つである。起動側（`start-review-manager.js`）はこのファイルを書かない。`launchAgentHeadless()` が返すのは shim（起動用の中間プロセス）のPIDであり、Review Manager 本体ではないため、生存判定にも稼働状況表示にも使えない。起動から本体が書き込むまでの窓は `manager.starting` の起動予約（`fs.writeFileSync(..., { flag: 'wx' })` による原子的な排他作成）で塞ぐ。所有者確認を伴う解放を持つロックファイルに、書き込み主体を2つ以上作らないこと（PR #410 Review Manager指摘）。
