...
経緯: プロジェクト内の5ファイル（`inbox-supervisor.js`・`msg-poll.js`・`publish-plan.js`・`assistant-watch.js`・`worker-exit-hook.js`）がそれぞれ同等の `_ghApiComments` / `_ghListComments` / `_ghIssueComments` 関数を持ち、うち3ファイルが `parseCommentsResponse` と同名の同一実装を独立に持っていた（Issue #183 で一掃）。
