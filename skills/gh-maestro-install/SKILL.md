...
5. 今回の install に `scripts/` または `skills/agents.yaml` の変更が含まれるなら、**セッションを終了する**（下記）

## `scripts/` を変更した install の後はセッションを再起動する（必須）
...
`scripts/` 配下または `skills/agents.yaml` に触れた場合は、**必ずセッションを終了し
orchestrator を再起動すること。** 変更ファイルが常駐4種の require 閉包に入るかは毎回
判定しない（判定コストの方が高い）。

**`skills/**` のドキュメントだけを変更した場合は再起動不要。** 常駐プロセスは SKILL.md を
