...
2. **作業環境を準備する**: `$WORKTREE` 内に `package.json` が存在し `node_modules` がない場合：
   ...
   - `$WORKSPACE/node_modules` も存在しない → orchestratorに `node "$WORKSPACE/.gh-maestro/scripts/send-pane.js" orchestrator --workspace $WORKSPACE "$WORKSPACE で npm install が必要です。実行後に再開してください"` と報告して待機する
