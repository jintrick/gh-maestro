...
- **追加指示の待ち受けは、通信ルールに注入された `{{INBOX_POLL_MECHANISM}}` の手順だけを使用すること。** 独自の background bash・detached プロセス・ポーリングループを作ってはならない。共通の `msg-poll.js` 側にライフサイクル管理（dead-man's switch + PID registry）が実装されている。
