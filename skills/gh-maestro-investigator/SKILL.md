...
報告を行った時点で初期の調査作業は完了するが、orchestratorから後続の追加調査指示や明示的な終了指示を受信するまでは、インボックスメッセージの受信を継続し、待機を維持しなければならない。
以下を実行することがゴールだ：
...
ゴールのコマンドを実行する。**実行したら自己終了（TaskStopなど）を行わず、そのまま待機状態を維持すること**。orchestratorが報告内容を元に後続の指示を送る可能性があるため、明示的な終了指示を受信するまでインボックスメッセージの受信を継続する（要約や念押しの地の文を追加せず、コマンド引数のみで報告する）。報告内容：
