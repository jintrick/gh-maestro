...
ゴールのコマンドを実行する。**実行したら自己終了（TaskStopなど）を行わず、そのまま待機状態を維持すること**。orchestratorが報告内容を元に後続の指示を送る可能性があるため、明示的な終了指示を受信するまでインボックスのポーリングおよび監視ループを維持する（要約や念押しの地の文を追加せず、コマンド引数のみで報告する）。報告内容：
