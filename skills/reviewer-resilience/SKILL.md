...
ユーザーがPR番号・URL・ブランチ名・Issue番号のいずれかで対象を指定する。
指定がない場合は「PR番号またはIssue番号を指定してください」と返す。

Issue番号が渡された場合は、まず対応するPRを特定してからレビューする：

```bash
# ブランチ名パターンで検索
gh pr list --search "head:issue-<N>" --state open --json number,title -q '.[0]'
# 見つからなければ本文検索
gh pr list --state open --json number,title,body -q '.[] | select(.body | strings | contains("#<N>")) | {number,title}' | head -1
```

PRが見つからない場合は「Issue #N に対応するPRがまだ作成されていません」と返す。
