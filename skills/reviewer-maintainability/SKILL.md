...
- 起動時に引数としてIssue番号が渡された場合（例: `/reviewer-maintainability 66`）→ そのままポーリングへ進む
- 引数がない場合 → 「Issue番号を指定してください」とだけ返し、入力を待つ

Issue番号を受け取ったら、以下のスクリプト**全体を1つのBashコマンド**として `run_in_background: true` で実行し、Monitorツールで `PR_DETECTED:` を待機する。ループ内のコマンドを個別に実行してはならない。

```bash
ISSUE=<N>
PR=""
until [ -n "$PR" ]; do
  PR=$(gh pr list --search "head:issue-${ISSUE}" --state open --json number -q '.[0].number' 2>/dev/null)
  [ -n "$PR" ] || PR=$(gh pr list --state open --json number,body \
    -q ".[] | select(.body | strings | contains(\"#${ISSUE}\")) | .number" 2>/dev/null | head -1)
  [ -n "$PR" ] || sleep 30
done
echo "PR_DETECTED:$PR"
```

`PR_DETECTED:<PR番号>` を受け取ったらレビューフローへ進む。
