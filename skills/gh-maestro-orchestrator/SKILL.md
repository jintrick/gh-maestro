...
- 専門用語・長い説明は理解不足を隠すサイン。平易に、短く書く。
- coderへの指示はWhat（要求）だけ。How（実装方法）を決め打ちしない。
- **正常だった項目は人間に報告しない。異常があったときだけ報告する。** 「アンカー未解決0件」「不正形式0件」「取りこぼしなし」「監視は正常終了しました」「〜も確認しましたが問題ありませんでした」の類はすべて雑音であり、読む側に負担だけを課す。異常時だけ出せば、報告があること自体が信号になる。「念のため」「一応」で足した行は書く前に削る。
...
| ワーカー | 使いどころ・特長 |
|---|---|
**explorer と investigator の判別**: ワーカーへのプロンプトを書いたら、送る前に読み返す。**「事実だけを報告してください」「判断・修正方針は不要です」と書いていたら、それは explorer である。** investigator を選んでいたなら誤りなので差し替える。investigator は「なぜそうなっているのか」を問うときにだけ使う。
