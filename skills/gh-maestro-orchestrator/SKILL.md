...
|---|---|---|---|---|---|---|
| 1 | <🔴BLOCKER/🟡MAJOR/🟢SUGGESTION> | <path>:<line> | <要約（2〜3文以内）> | <命名の異常/バグ・セキュリティ/エッジケース/SUGGESTION> | <N>/5 — <一言理由>（バグ・セキュリティ／エッジケース）／<低/中/高> — <一言理由>（SUGGESTION）／—（命名の異常） | <コーダーに修正依頼/無視/保留Issueに積む> |

対応しますか？番号で指定してください（例:「1は修正依頼、2は無視」）。
```

各指摘の実行（修正依頼／無視／保留Issueに積む）は、テーブル全体の提示後、人間の回答（番号ごとの指定）を待ってから行う。
