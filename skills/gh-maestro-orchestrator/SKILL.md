...
architect の検討結果は Issue のコメントとして記録される。コーダーへの実装指示は、コーダー自身が計画フェーズで作成するpin済み計画コメントであり、orchestrator が Issue 本文に実装手順を統合する工程は廃止された（下記「計画評価と承認」参照）。
...
architect の計画レビュー結果（承認推奨または要修正）は、あくまで推奨であり、最終承認は必ず人間が行う。レビュー結果を人間に提示する際は、architect の評価が推奨であることを明示し、人間の判断を仰ぐ。
...
Issueの読者は**承認判断を行う人間**だけである。実装の詳細（対象ファイル・変更方針・作業分割・検証条件）はコーダーが計画フェーズでpin済みコメントとして作成するため、Issue本文には「何を実現したいか」（目的・振る舞い・制約・対象外・受け入れ条件）だけを書く。
...
- 「実装詳細」（対象ファイル・変更方針・作業分割・検証条件）はコーダーが計画フェーズでpin済みコメントとして作成する
...
調査アンカーとして使っていた既存Issueがある場合はそのIssueを更新し、全く新規の作業で調査不要な場合は新規作成する。Issue本文は「概要（人間向け）」のみで構成し、実装詳細は含めない（実装詳細はコーダーの計画フェーズでpin済みコメントとして作成される）。
...
**この inbox 監視（`msg-poll.js orchestrator`）はセッション中に最初の1回だけ起動する。** 以後、待ちたい相手や場面（PR検出・レビュー監視・ワーカー起動待ち・本番公開（CI/CD）確認・反省会での応答待ちなど）が変わっても、新しいMonitorを起動し直さず、既存の1本をそのまま使い回す。
...
PRに新しいレビューコメントが届くたびに、orchestratorは指摘を分類・評価するところまでを行う。**実際にコーダーへ転送する／フィルターする／保留Issueに積むという実行アクションは、分類やスコアの値に関わらず、必ず人間の明示的な決定を経てから行う。** 「バグ・セキュリティだから」「命名は機械的判定だから」という理由で人間の確認を省略してよい経路は存在しない。
...
### エッジケース確信度スコア（分類が「バグ・セキュリティ」または「エッジケース」の指摘に付ける）

人間が転送するか・フィルターするかを判断するための、その場限りの提示材料。記録には残さない。
...
【レビュー指摘確認】 PR #$PR
指摘: <path>:<line> — <要約>
分類: <命名の異常/バグ・セキュリティ/エッジケース/SUGGESTION>
エッジケース確信度: <N>/5 — <理由>（バグ・セキュリティ／エッジケースの場合のみ）
対応コスト: <低/中/高> — <理由>（SUGGESTIONの場合のみ）
オーケストレーターの推奨: <転送する/フィルターする/保留Issueに積む>
対応しますか？
- 転送する（コーダーに修正依頼）
- フィルターする（無視）
- 保留Issueに積む
...
複数の指摘が同じタイミングでまとまって届いた場合（初回レビュー完了直後など）は、1件ずつ提示せず、指摘ごとの内容を並べた1つのメッセージにまとめて提示してよい。その場合も、各指摘の実行は人間の回答を待ってから行う。
...
- **転送する**:
  ...
  ```
  同一トリアージサイクルで他にも「転送する」と決まった指摘があれば、まとめて1回のメッセージで送ってよい。
- **フィルターする**: 何もしない。
- **保留Issueに積む**:
  ```sh
...
| 優先 | 分類 | 判断基準 | 提案先 |
|---|---|---|---|
| 1 | **型・制御フローで構造的に防止可能** | 既存の言語機構（rethrow・非null型・網羅性チェック・不変条件を持つ型設計等）で、そのバグを実装として書けなくできる。コーダーが「ルールを読んで守る」ことに依存しない | 該当コードの構造修正 Issue（investigator起動で影響範囲・他の呼び出し経路を特定） |
...
人間が承認した項目について：
- **型・制御フローでの防止**: investigator を起動して該当コードと他の呼び出し経路を含む影響範囲を特定し、新しい Issue として起票する
- **Lint化**: investigator を起動して設定ファイルを特定し、新しい Issue として起票する
- **テスト追加**: investigator を起動して該当テストファイルを特定し、新しい Issue として起票する
