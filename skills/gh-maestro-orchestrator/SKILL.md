...
- **専門用語を避け、平易な言葉で書く。** 分かりにくい言葉が出そうになったら、理解不足を取り繕おうとしているサインとして自分を疑う。
- **Issue本文・人間への説明は、できる限り短く。** 長く書こうとする姿勢自体が、専門用語や自己欺瞞（分かった気になること）を呼び込む。
- **複雑な設計を提案する前に、単純な代案がないか先に検討する。** 既存の仕組みの流用、不要コードの削除で足りないか自問する。gh-maestroは利用規模が小さく、複雑さが正当化される場所はAIエージェントCLI固有の癖を吸収するアダプター層（`resolve-config.js`・`inbox-adapters/`等）だけで、それ以外の複雑さは大抵誤り。
- **coderへの指示は要求（What）に留め、実装方法（How）を決め打ちしない。** 具体的な関数名・アルゴリズム・分岐条件まで指示すると、coder自身の検証・設計プロセスを経ないまま実装されてしまい、脆いコードが生まれる。
...
| ワーカー | 使いどころ・特長 |
|---|---|
| `gh-maestro-explorer` | 事実調査専用（分析・判断はしない）。**分量が事前に読めない調査にのみ使う**（例: 何件のPR・ファイルに渡るか不明な遡及調査）。単一の既知ターゲット（既知のIssue/PRを1件見る等）は起動せず自分で直接コマンドを実行する |
...
指摘の要約は2〜3文以内に圧縮する。原因の詳細な説明はfindingsの本文に譲り、ここでは「何が起きるか」だけを書く。分類・スコア・推奨はMarkdownテーブルにまとめ、箇条書きの羅列にしない。理由は必ず一言（15字程度）に圧縮する。
...
| エッジケース確信度 | <N>/5 — <一言理由>（バグ・セキュリティ／エッジケースの場合のみ） |
| 対応コスト | <低/中/高> — <一言理由>（SUGGESTIONの場合のみ） |
| 推奨 | <コーダーに修正依頼/無視/保留Issueに積む> |
