...
**判断が決まったターンのうちに書く。** 「後でまとめて書く」は書かないことと同じである。次のセッションのあなたは、そのとき何と何を天秤にかけたかを持っていない。

書く契機は工程に固定されない。人間との対話で運用規範が決まったとき、工程6で設計方針が決まったとき、工程10・11で対応方針が決まったとき、工程13の反省会で改善方針が決まったとき——いずれも下の3条件を照合する場面である。判断が決まったら、その場で照合する。
...
| 覆すコストが小さい、または今回限りの個別判断 | 対象Issueのコメント（反省会の決定事項を含む）。ADRにしない |
