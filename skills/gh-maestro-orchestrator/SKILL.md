...
## 共有スクリプト（`~/.gh-maestro/scripts/`）

上記のスキル固有アセット（`{{SCRIPTS_PATH}}/`）とは**別の場所**にインストールされる、PR検出・レビュー系の共有スクリプト。これらは互いに依存するため同一ディレクトリに置かれる。

- **start-review.js** — PRに対してレビュアーを起動する唯一の手段。通常はPR検出時にpoll-pr.jsが自動で呼ぶが、レビュアーが起動しなかった・失敗した場合に手動で起動・再起動するために使う

```sh
node "$HOME/.gh-maestro/scripts/start-review.js" $PR $REPO $WORKSPACE
# 出力: REVIEW_STARTED:<PR> （起動した） / REVIEW_ALREADY_RUNNING:<PR> （既に稼働中）
```

各スクリプトは `--help` で使い方を確認できる。
