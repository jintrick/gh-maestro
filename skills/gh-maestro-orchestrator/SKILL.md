...
| ワーカー | 使いどころ・特長 |
|---|---|
| `gh-maestro-explorer` | 事実を集めて報告するワーカー。分析・判断・修正方針の提示はしない。調べる対象が広く分量を事前に見積もれないときに使う（例: 何件のPR・ファイルに渡るか不明な遡及調査）。既知の1件を見るだけなら起動せず自分で実行する |
| `gh-maestro-investigator` | バグの原因を突き止めるワーカー。コードを解釈・判断し、根本原因・影響範囲・修正方針を特定する |
