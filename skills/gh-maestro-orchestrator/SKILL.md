...
architect は設計コメントの URL だけを既存の `msg-send.js` 経路で orchestrator に通知する。通知を受けたら URL の Issue コメントを読み、設計本文をメッセージ本文として受け取ったものと扱わない。不足情報・要件矛盾なら必要な調査または人間確認を行う。追加の設計成果物が必要なら、現在の architect は inbox で待機させたまま、新しい実行IDで architect を起動する。

設計コメントの投稿成功は実行記録の完了であり、architect ワーカーの終了ではない。orchestrator は inbox を維持したままの architect を、明示的な終了指示を送るまで終了・削除してはならない。
