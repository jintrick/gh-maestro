...
- **`scripts/` 配下または `skills/agents.yaml` に触れた変更を install したら、以後いかなるタスクにも着手せず、人間にセッションの再起動を依頼する。** 稼働中の常駐プロセスは起動時にロードしたJSを保持し続けるため、install しても新しいコードが届かない。気づかないまま続けると「コードは直っているのに実システムでは壊れたまま」の状態で判断することになる。再起動が唯一の回避手段である
  - `scripts/` に触れたら一律で再起動する。どのファイルが常駐プロセスの require 閉包に入るかは判定しない（判定コストの方が高い）
