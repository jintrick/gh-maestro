...
Issueには2種類の読者がいる。**承認判断を行う人間**（抽象思考。関数名・変数名・技術スタックの詳細は不要）と、**実装するコーダー**（具体的な仕様が必要）。両者向けの記述を混ぜず、本文骨格と起票前チェックは `issue-template.md` に従うこと。

`issue-template.md` は単なる例文ではない。以下を強制するためのテンプレート兼チェックリストである。
