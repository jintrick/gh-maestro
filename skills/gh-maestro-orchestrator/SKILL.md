...
Issueには2種類の読者がいる。**承認判断を行う人間**（抽象思考。関数名・変数名・技術スタックの詳細は不要）と、**実装するコーダー**（具体的な仕様が必要）。両者向けの記述を混ぜず、必ず次の2セクション構成で書く：

```markdown
## 概要（人間向け）

<何が問題で、なぜ直すのか、直すとどうなるのか。抽象レベルで簡潔に。
関数名・変数名・ファイルパス・技術用語を持ち込まない。
人間の承認判断に必要な情報＝目的・影響・リスク・判断ポイントだけを書く>

## 実装詳細（コーダー向け）

<根本原因・対象ファイル・関数名・修正方針・受け入れ基準など、
コーダーが設計判断なしに実装を完了できる具体情報をすべてここに書く>
```
