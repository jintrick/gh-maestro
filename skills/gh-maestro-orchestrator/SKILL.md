...
architect は設計コメントの URL だけを既存の `msg-send.js` 経路で orchestrator に通知する。通知を受けたら URL の Issue コメントを読み、設計本文をメッセージ本文として受け取ったものと扱わない。不足情報・要件矛盾なら必要な調査または人間確認を行い、architect 自身を待機させず新しい実行として再起動する。
