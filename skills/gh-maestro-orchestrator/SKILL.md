...
`gh-maestro-coder` のワーカーを初めて起動する直前に、`[gh-maestro available agents]` に列挙されたエージェントをユーザーに提示し、使用するエージェントを1つ選択させる。選択されたIDを `SELECTED_AGENT` 変数として記憶し、以降のすべての `spawn-worker.js` 呼び出しに `--agent $SELECTED_AGENT` を付与する。2回目以降のワーカー起動では再度聞かない。
...
各指摘を以下の軸で分類し、**構造的に防げるものだけ**提案する：

| 分類 | 判断基準 | 提案先 |
|---|---|---|
| **Lint化可能** | ESLint / Prettier / 型チェック等のルール追加で機械的に検出できる | lint 設定ファイルの更新 Issue |
| **コーダールール化可能** | コーダーの初期実装方針として明文化すれば次回から発生しない | `gh-maestro-coder` SKILL.md の更新提案 |
| **Issue記述の改善** | Issue に制約や非機能要件を明記していれば実装前に判断できた | Issue テンプレートまたは起草フローの改善提案 |
| **個別判断が必要** | プロジェクト固有の設計判断で、汎用化できない | 記録のみ（提案しない） |
...
- **Issue記述改善**: 次回の Issue 起草時に自分のチェックリストとして保持する
