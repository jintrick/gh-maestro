...
## ワーカーの停止検知通知（Inbox Supervisor 自身による検知）

Inbox Supervisor は、`workers.json` に登録されたワーカーのプロセスが生存していないことを検知した場合、停止通知（`⚠️ ワーカー "<name>" のプロセス（PID ...）が停止しています。報告状況: ...`）を orchestrator に通知する。

orchestrator は通知本文の報告状況（報告済み／未投稿／判定不能（起動時刻なし・不正））をそのまま読む（GitHub を照会し直さない・推論しない）。

- **「報告済み」の場合**: 正常に完了して終了した状態であるため、何もしない（人間に報告しない）。
- **「未投稿」の場合**: 作業が途中で停止した可能性があるため、以下の手順で確認して人間に報告する。
- **「判定不能（起動時刻なし・不正）」の場合**: 起動時刻が記録されていないため報告有無が自動判定できない。起動情報欠落の事実を人間に伝え、ログ等から完了状況を確認する。

確認・対処手順:
1. **生死の再確認**:
   `node "{{SCRIPTS_PATH}}/worker-status.js" status --workspace $WORKSPACE --worker-name <workerName>` で実際に停止しているか確認する。
2. **進捗・停止位置の確認**:
   `$WORKSPACE/.gh-maestro/worker-logs/<workerName>.log` の末尾を読み、どこまで進んで停止したかを把握して人間に事実を報告する。
3. **未配送メッセージの確認**:
   休止中に受信したメッセージや指示が未配送のまま残っていないか確認する。
4. **resume（再開）の手順**:
   orchestrator が独断で勝手に再開・再起動・削除を行ってはならない。再開するかどうかは人間の判断を仰ぐ。再開指示があれば、`msg-send.js` で指示を送ることで Inbox Supervisor が自動的にワーカーを再開（resume）して配送する。
