...
```sh
eval $(node "scripts/get-context.js")
...
```sh
WORKER=$(node "scripts/spawn-worker.js" \
...
```sh
node "scripts/send-pane.js" $WORKER "<返答内容>" --workspace $WORKSPACE
```

### ワーカーの終了とworktree削除

```sh
node "scripts/remove-worker.js" \
  --worker-name $WORKER \
  --workspace $WORKSPACE
```
