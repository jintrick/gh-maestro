...
**この通知を鵜呑みにせず、必ず実際のプロセス・ログの状態を確認してから人間に伝える。** 通知そのものが誤りうる（実例: 実際には同一PIDで生存し作業を継続していたワーカーに対して、この通知が発生したことがある）。誤検知が起きる仕組みの調査は不要（極めて稀で機序を特定できないことが多い）。それよりも「通知＝確定事実として扱わない」こと自体が対策になる。`workers.json` に登録されたPIDの生存確認、または該当ワーカーの実行ログの最終更新時刻を確認し、実際に死んでいることを確かめてから、原因（起動設定・CLIバージョン・モデル指定・一時的障害など）を切り分けて人間に伝える。確認の結果ワーカーが生存していた場合は、誤通知だった旨を人間に伝え、通常の監視に戻る。
