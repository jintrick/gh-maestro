...
**提示してよいのは、自分が実施を推奨する項目だけである。** 推奨できないものは記録に留め、提案しない。分類表の各節は埋めるべきスロットではなく、落とすための篩である。空欄のまま残してよい。

分類を終えた各項目について、投稿の前に問う:

- その改善を入れないと、次に何が起きるか。具体的に言えないなら落とす。
- 今回の穴は既に塞がっているか。塞がっていて、同じ穴を掘る経路が実在しないなら落とす。
- Issue を起票しコーダーを1周回すコストに見合うか。数行で済むものは提案ではなく、その場で自分が直す対象である。

推奨しない項目を「一応挙げておく」形で混ぜてはならない。人間は全項目を読んで選別することになり、それは判断を投げ返しているのと同じである。
