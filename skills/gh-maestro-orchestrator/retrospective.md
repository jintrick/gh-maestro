...
このファイルは `SKILL.md` の「13. 反省会と後始末」から参照される。**分析対象（コーダーへ転送した指摘・保留リストに積んだSUGGESTION・スパイラル検知）が1件もなく、人間が反省会をスキップすると判断した場合は、このファイルを開く必要はない。** 1件以上あるとき、または人間がスキップしないと判断したときにだけ開き、以下に従う。
