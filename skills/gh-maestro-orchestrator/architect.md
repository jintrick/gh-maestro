...
1. **抽象設計の検討**（既存）: 具体的な実装手順を作るための必須ゲートではない。コードを読まなければ結論を出せない具体化を任せる先でもない。確定済み要件、相談したい設計上の論点、参照先のポインタだけで扱える、抽象的な設計判断の価値がコストを上回る場合にだけ起動する。
...
確定済み要件（Issue 本文）、相談したい設計上の論点、参照先のポインタ（Issue コメント番号・ファイルパス）が揃い、architect がサブエージェント経由で必要な事実を取得でき、かつ「Architect 起動判断」を満たす場合にだけ `gh-maestro-architect` を起動する。生の会話ログや未選別の探索結果を渡してはならない。

architect のコメントは設計検討の記録であり、要件定義を変更する根拠にはならない。コメントに不足情報や矛盾の指摘があれば、必要な explorer/diagnostician の再調査または人間への確認を行い、相談したい設計上の論点と参照先のポインタを更新して architect を再実行する。要件本文を変更できるのは人間との合意がある場合だけである。
...
node "{{SCRIPTS_PATH}}/spawn-worker.js" \
  ...
  --prompt-file <確定済み要件・相談したい設計上の論点・参照先のポインタを含むファイル> \
