...
5. **新規追加・修正したファイルに対応するテストケースを作成し、`npm test` を全件実行する**
   ...
   - テスト層が分かれているプロジェクトでは、実行範囲を次の原則で決める。重要度、壊れやすさ、担当者の判断で範囲を変えてはならない
     - 反復中は毎回側だけを実行する
     - 変更したものと名前が一致する分離側テストがあれば、それも実行する。名前が一致するものが無ければ分離側を全件実行する
     - 層の名前、実行コマンド、対応規則は、そのプロジェクトの規約に従う。この指示書は特定のディレクトリ名やコマンド名を定めない
   - 層が分かれていないプロジェクトでは、そのプロジェクトの標準のテストコマンドを全件実行する
