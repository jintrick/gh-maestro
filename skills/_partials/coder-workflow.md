...
6. **初回実装完了時・レビュー指摘対応の修正push時に、ステージング・コミット・push・PR取得/作成・テスト結果申告を一つの操作にまとめた `push-and-declare.js` を実行する**:
   ...
   - 申告は直前のテスト runner が生成した成果物から fail/pass と実行範囲を読み取る（テストが赤でも終了コードは0）。宣言された毎回側の成果物が欠落・破損・対象外の場合、`push-and-declare.js` は push 後に非ゼロ終了する。文書だけの変更はテスト結果を要求せず、従来どおり進む。数値を手入力して補ってはならない
