...
5. **新規追加・修正したファイルに対応するテストケースを作成し、宣言された毎回側のランナーを全件実行する**
   ...
   - テスト層が分かれているプロジェクトでは、`test.layers` の宣言から層名・実行コマンド・変更ファイルとテストの対応規則を読み、プロジェクトの規約に従って実行する。重要度、壊れやすさ、担当者の判断で範囲を変えてはならない
   ...
   - 実行は宣言されたランナー経由で行い、成果物に残った終了コード・実行範囲・件数を `push-and-declare.js` に引き渡す。結果の数値を手入力してはならない
     - 実行入口は `node "{{SCRIPTS_PATH}}/run-tests.js" --workspace $WORKSPACE <宣言された層名>` とし、分離側の変更対応を使うときは `--changed` と変更ファイルの相対パスを渡す
