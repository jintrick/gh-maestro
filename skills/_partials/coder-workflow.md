...
5. **新規追加・修正したファイルに対応するテストケースを作成し、宣言された毎回側のランナーを全件実行する**
   ...
   - 実行は宣言されたランナー経由で行い、成果物に残った終了コード・実行範囲・件数を `push-and-declare.js` に引き渡す。毎回側の実行では lint も同じ成果物へ記録する。lint の指摘はテストの失敗や終了コードへ変換しないが、lint 記録が欠落・古い・破損している場合は申告を完了させない。lint 記録の欠落だけで壊れた成果物と扱わず、`run-tests.js` の再実行を促す。結果の数値を手入力してはならない
...
6. **初回実装完了時・レビュー指摘対応の修正push時に、ステージング・コミット・push・PR取得/作成・テスト結果申告を一つの操作にまとめた `push-and-declare.js` を実行する**:
   ...
   - 申告は直前のテスト runner が生成した成果物から fail/pass、実行範囲、lint記録を読み取る（テストが赤でも終了コードは0）。宣言された毎回側の成果物または lint 記録が欠落・古い・破損・対象外の場合、`push-and-declare.js` は push 後に非ゼロ終了する。文書だけの変更はテスト結果を要求せず、従来どおり進む。lintの指摘自体は停止条件にせず、数値を手入力して補ってはならない
