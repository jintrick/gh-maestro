...
5. **新規追加・修正したファイルに対応するテストケースを作成し、`npm test` を全件実行する**
   - `npm test` はリポジトリ内の test runner が出力を集計し、worktree 外の runtime root に機械可読なテスト結果成果物を自動生成する。fail/pass の数値を手で数えたり、申告用に入力したりしてはならない
   ...
   - テスト層と分離側の実行先は、次の機械的な規約で決める。重要度、壊れやすさ、担当者の判断で範囲を変えてはならない
     - `npm test` は毎回側であり、通常の実装変更および `tests/*.test.js` の変更では全件実行する
     - `scripts/<name>.js` を変更したら、`tests/slow/<name>.test.js` の存在をファイル名の一致だけで確認する。存在する場合は `npm run test:slow -- tests/slow/<name>.test.js` を実行し、存在しない場合は `npm run test:slow` を全件実行する
     - 複数の `scripts/*.js` を変更した場合は、存在する同名の分離側テストをすべて個別に実行する。同名が1つでも存在しない場合の全件実行は1回だけ追加する
     - `tests/slow/<name>.test.js` を直接変更した場合は、そのファイルを `npm run test:slow -- tests/slow/<name>.test.js` で実行する。分離側テストだけを変更した場合、変更したテスト自身の実行で足りる
     - 分離側へ置く基準は、実プロセス起動、実git操作、実シェル起動、実時間待ちのいずれかを含むことだけとする。これらを含まないテストを重要度で分離側へ移してはならない
     - `node scripts/install.js` の前、障害調査時、および同名テストが存在しない変更時だけ、分離側を `npm run test:slow` で全件実行する。インストール前は `npm test` とこの全件実行の両方を完了させる
