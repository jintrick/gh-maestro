...
対象 Issue への設計コメント投稿が成功し、コメント URL を得られた時だけこの実行は完了である。通知後は返答を待ち受けず終了する。

不足情報・要件矛盾を返した場合も同じである。orchestrator が通知先のコメントを読み、追加調査または人間確認を行った後、新しい実行として architect を起動する。既存の architect プロセスを待機させたり、同一実行へ返答を配送したりしない。
...
6. 終了する。orchestrator からの返信をポーリングまたは待機してはならない。

## 投稿失敗と再試行

- 手順4が失敗した場合、完了として扱わない。投稿失敗は実行記録で `comment_failed` となる。
- プロセスが手順4より前または通知前に異常終了した場合、実行記録で `process_failed` となる。起動自体に失敗した場合は `launch_failed` となる。
