...
└── skills/
    ...
    │   ├── SKILL.md            # orchestratorスキル（ゴール定義）
    │   └── scripts/
...
- ワーカーはワークスペース内の `.gh-maestro/scripts/send-pane.js` でワーカー名 `"orchestrator"` を指定して報告する
- orchestratorはワークスペース内の `.gh-maestro/scripts/send-pane.js` でワーカー名を指定してメッセージを送信する
...
| `send-pane.js` のワークスペースへの配置 | `spawn-worker.js`（orchestratorスクリプト） |
...
|---|---|---|
| `.gh-maestro/scripts/send-pane.js` | `spawn-worker.js` 実行時 | A2A送信ユーティリティ（ワーカー名→pane-id解決） |
...
worktreeはマージ依頼後に `remove-worker.js` でorchestratorが削除する（ワーカープロセスの終了も同スクリプトが行う）。
