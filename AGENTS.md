...
- gh-maestroはAIエージェント開発の基盤である。速度や手軽さより堅牢性を優先する。構造的な脆弱性を見つけたら、その場で握り潰さず記録して起票を相談する。理由と経緯: `docs/adr/0023-robustness-over-speed.md`
...
- 複雑な設計案が浮かんだらまず疑う。既存の仕組みの流用や不要コードの削除で済まないかを先に検討する。複雑さが正当化されるのは、AIエージェントCLI間の差異を吸収するアダプター層だけである。オーケストレーション本体に複雑さを持ち込まない。理由と経緯: `docs/adr/0024-complexity-only-in-agent-adapter.md`
