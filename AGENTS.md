...
- コミットの結果は、フックやスクリプトが出したログではなく`git show --stat`で実際の内容を確認する。「同期しました」等の出力は結果の証拠にならない。
...
- WezTermへの依存を新たに増やさない。エージェント間の受け渡しはファイルを正本とし、受信側が読みに行く。通知に本文を載せない（受信側の入力トークンを浪費する）。
...
- ツールが「拒否された」と表示されても、副作用が無かった保証にはならない。投稿・書き込みが済んでいた実例がある。結果を確認してから報告する。
