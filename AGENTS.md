...
- gh-maestroはAIエージェント開発の基盤である。速度や手軽さより堅牢性を優先する。構造的な脆弱性を見つけたら、その場で握り潰さず記録して起票を相談する。
...
- ガードはAIの事故を防ぐためにある。同じマシンの人間が意図的に迂回することは想定しない。ローカルシェルから状態ファイルやトークンを読めることを根拠に、ガードが無効だと判定しない。理由と経緯: `docs/adr/0029-spawn-guard-is-not-a-security-boundary.md`
...
- 用語の定義は`GLOSSARY.md`にある。曖昧な言い換えを避けるため、そこにある語はその意味でのみ使う。
- 既定のエージェント設定を変更するときは、`skills/agents.yaml`の更新を優先する。
- 実行可能な処理はスキルディレクトリに隠さず、`scripts/`に置く。
- gh-maestro自身を開発する場合、プロジェクトローカルの`.claude/skills/`がユーザー領域より優先されるため、`install.js`を実行しなくてもスキルの変更が反映される。`install.js`は他プロジェクトへの配布のためにある。
...
質問を1つに見せるために疑問符の数だけを数えてはならない。「はい」の返答で実行する操作を数え、複数あるなら発言を分ける。
...
- コミットの結果は、フックやスクリプトが出したログではなく`git show --stat`で実際の内容を確認する。「同期しました」等の出力は結果の証拠にならない（同期ログを見て報告したが、フックが`git add`しておらず2コミット分が漏れていた事故がある）。
...
- 複雑な設計案が浮かんだらまず疑う。既存の仕組みの流用や不要コードの削除で済まないかを先に検討する。複雑さが正当化されるのは、AIエージェントCLI間の差異を吸収するアダプター層だけである。オーケストレーション本体に複雑さを持ち込まない。
- WezTermへの依存を新たに増やさない。エージェント間の受け渡しはファイルを正本とし、受信側が読みに行く。通知に本文を載せない。理由と経緯: `docs/adr/0025-file-as-source-of-truth-not-wezterm.md`
- 規範から ADR への参照行（`理由と経緯: docs/adr/…` および `判断の背景と採用理由: docs/adr/…`）を削除しない。この行は、その規範がなぜそうなっているかを辿る唯一の経路である。規範の本文を書き換えるときも参照行は残す。消してよいのは規範ごと削除するときだけであり、参照先を変えてよいのは ADR が覆されて張り替えるときだけである（`docs/adr/` の扱いは `skills/gh-maestro-orchestrator/adr.md`）。
- 既存パターンを惰性で踏襲しない。粗末な実装を無断で行わない。編集される内容はコード内にハードコードせず、編集しやすいファイルへ置く。
- 対策を打ったことをゴールにすり替えない。受け入れ条件と1項目ずつ照合し、実証してから完了とみなす。「設計した」「修正した」という報告だけで完了扱いにしない。
- ツールが「拒否された」と表示されても、副作用が無かった保証にはならない。結果を確認してから報告する（拒否表示のまま投稿済みだったのを確認せずに報告し、無実のワーカーに責任を負わせた事故がある）。

## 実装規範

`.claude/rules/` は、対象のファイルを開いたときに文脈へ流し込まれる補助である。`paths` が実質すべてに一致する規範をそこに置くと、AGENTS.md に書くのと同じ効果しか持たないまま、ファイルを読むたびに繰り返し流し込まれる。以下はどのファイルにも掛かる規範なのでここに置く。

### CLIスクリプトの引数と `--help`

- 引数解析は `scripts/shared/workspace.js` の `parseFlags` を使う。生の `argv.includes('--help')` でフラグを判定しない（他のオプションの値がたまたま `--help` と一致すると誤判定する）。`parseFlags` を catch したときのヘルプ判定は `err.helpRequested` を使う。
- 位置引数の個数を検証し、想定外なら無言で無視せずエラーにする。
- `--stdin` 等の標準入力待ちオプションは `process.stdin.isTTY` を確認し、TTYなら即エラーで終了する（対話的に呼ばれるとハングするため）。
- エージェントが CLI として呼びうるアセットは `--help` / `-h` を実装し、終了コード0で終わる。
- フラグの追加・削除・意味変更、引数の必須/任意の変更、出力フォーマットの変更を行うときは、**同じコミットで `--help` の Usage も更新する。** orchestrator とワーカーは SKILL.md ではなく `--help` を見て使い方を確認するため、`--help` が実装の挙動を表す唯一の一次情報である。

### 安全ガードはフェイルクローズにする

- 外部コマンドの実行結果に依存する安全ガードは、検出そのものが失敗したとき（コマンドが無い、例外を投げた等）、警告して継続せず、`--force` 等の明示的なフラグなしでは中断する。検出失敗は「安全と判定できない」ことを意味する。
- フェイルクローズを意図して throw する関数の呼び出しを、空の catch で囲まない。
...
エージェントCLIの起動方法（フラグ・サブコマンド・引数の組み合わせ）を変更・追加したときは、コミット前に実際にそのコマンドを実行して終了コードを確認する。`--help` でフラグ一覧を見るだけでは足りない。既存の設定値をそのまま使う薄いラッパーであっても、実際にCLIを起動する新しいコードパスである限り対象である。

### テストの書き方

- **期待値を検証対象から導出しない。** 自己整合性の確認にしかならず、対象が丸ごと欠落しても通る。期待値は検証対象と独立した出所から取る。
- **`workspace` に実リポジトリのパスを渡さない。** 一時ディレクトリを使う。書き込み・削除を伴う関数に実リポジトリのパスを渡さない。
- **子プロセス境界の argv を、常に注入モックで差し替えたままにしない。** 呼ばれる側のCLIが引数規約を変えても、呼ぶ側が旧規約のままでも両者のテストが緑になる。実際に組み立てる argv を検証するテストを残す。
- **同じ catch 経路へ落ちる複数の失敗種別は、代表1つで済ませない。** JSON構文エラー・型不正・読み取り失敗のように扱いが分かれうるものは、種別ごとに独立して注入して検証する。一部の種別だけが正常値へ縮退する変更が、既存テストを通過する。
